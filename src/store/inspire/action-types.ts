export enum ActionTypes {
  GetUserInvitationCodes = 'GET_USER_INVITATION_CODES',
  CreateUserInvitationCodeForAppOtherUser = 'CREATE_USER_INVITATION_CODE_FOR_APP_OTHER_USER',

  CreateAppCommissionSetting = 'CREATE_APP_COMMISSION_SETTING',
  UpdateAppCommissionSetting = 'UPDATE_APP_COMMISSION_SETTING',
  GetAppCommissionSetting = 'GET_APP_COMMISSION_SETTING',

  CreateAppPurchaseAmountSetting = 'CREATE_APP_PURCHASE_AMOUNT_SETTING',
  GetAppPurchaseAmountSettings = 'GET_APP_PURCHASE_AMOUNT_SETTINGS',
  CreateAppPurchaseAmountSettingForAppOtherUser = 'CREATE_APP_PURCHASE_AMOUNT_SETTING',

  GetCommissionCoinSettings = 'GET_COMMISSION_COINS_SETTINGS'
}
